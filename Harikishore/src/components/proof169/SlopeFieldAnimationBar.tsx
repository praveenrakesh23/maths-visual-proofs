...
const S_STATES: { id: ProofState; label: string; step: number; desc: string }[] = [
  { id: 'inspect', label: 'Inspect', step: 1, desc: 'Identify given ODE dy/dx = f(x, y)' },
  { id: 'manipulate', label: 'Manipulate', step: 2, desc: 'Drag initial condition P0 and adjust step size' },
  { id: 'preserve', label: 'Preserve', step: 3, desc: 'Verify local tangents match slope arrows' },
  { id: 'connect', label: 'Connect', step: 4, desc: 'Compare Euler steps to continuous solution' },
  { id: 'conclude', label: 'Conclude', step: 5, desc: 'Synthesize Fundamental Theorem limit h -> 0' },
  { id: 'transfer', label: 'Transfer', step: 6, desc: 'Explore circular, logistic, and nonlinear ODEs' },
];

export const SlopeFieldAnimationBar: React.FC<Props> = ({
  currentState,
  onSelectState,
  onOpenHints,
}) => {
  ...
  const currentIndex = S_STATES.findIndex(s => s.id === currentState);

  useEffect(() => {
    let timer: ReturnType<typeof setInterval>;
    if (isPlaying) {
      timer = setInterval(() => {
        const nextIdx = (currentIndex + 1) % S_STATES.length;
        onSelectState(S_STATES[nextIdx].id);
      }, 3500 / speed);
    }
    return () => {
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-5 border border-sky-100/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 self-end md:self-auto">
        <button
          onClick={() => onSelectState(S_STATES[Math.max(0, currentIndex - 1)].id)}
        ...
        <button
          ...
          className="p-2.5 rounded-2xl bg-sky-600 hover:bg-sky-700 text-white shadow-md shadow-sky-500/20"
        >
          {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 fill-white" />}
        </button>

        <button
          onClick={() => onSelectState(S_STATES[Math.min(S_STATES.length - 1, currentIndex + 1)].id)}
          disabled={currentIndex === S_STATES.length - 1}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 disabled:opacity-40"
        >
