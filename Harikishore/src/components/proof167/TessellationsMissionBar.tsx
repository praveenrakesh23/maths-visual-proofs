...
import { Target, Grid, Magnet, Eye, AlertTriangle, Sparkles } from 'lucide-react';
...
interface MissionBarProps {
  ...
  showAngles: boolean;
  setShowAngles: (val: boolean | ((prev: boolean) => boolean)) => void;
  showGaps: boolean;
  setShowGaps: (val: boolean | ((prev: boolean) => boolean)) => void;
  snapEnabled: boolean;
  setSnapEnabled: (val: boolean | ((prev: boolean) => boolean)) => void;
}

export const TessellationsMissionBar: React.FC<MissionBarProps> = ({
  showGrid,
  setShowGrid,
  showAngles,
  setShowAngles,
  ...
  return (
    <div className="bg-white rounded-2xl p-3.5 md:p-4 border border-emerald-100/80 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-800 tracking-tight font-sans">
            Repeat tiles by transformations and test for gaps and overlaps.
          </h3>
          <p className="text-xs text-slate-500 font-medium">
            A tessellation board shows translations, rotations, and reflections while tracking vertex-angle fit.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 self-end md:self-auto flex-wrap">
        ...
        {/* Vertex Angles Toggle */}
        <button
          onClick={() => toggle(setShowAngles)}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
            showAngles
              ? 'bg-emerald-50 text-emerald-700 border-emerald-200 shadow-sm'
              : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'
        ...
        {/* Gaps / Overlaps Toggle */}
        <button
          onClick={() => toggle(setShowGaps)}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
            showGaps
              ? 'bg-emerald-50 text-emerald-700 border-emerald-200 shadow-sm'
              : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'
