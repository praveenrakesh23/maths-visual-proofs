...
const G_STATES: { id: ProofState; label: string; step: number; desc: string }[] = [
  { id: 'inspect', label: 'Inspect', step: 1, desc: 'Identify 2D contour map f(x, y) = C' },
  { id: 'manipulate', label: 'Manipulate', step: 2, desc: 'Drag probe point P and evaluate partials' },
  { id: 'preserve', label: 'Preserve', step: 3, desc: 'Verify 90° orthogonality invariant ∇f · T = 0' },
  { id: 'connect', label: 'Connect', step: 4, desc: 'Project directional derivative D_u f = ∇f · u' },
  { id: 'conclude', label: 'Conclude', step: 5, desc: 'Synthesize max ascent rate ||∇f||' },
  { id: 'transfer', label: 'Transfer', step: 6, desc: 'Explore saddle, Gaussian, and elliptical ridges' },
];

export const GradientAnimationBar: React.FC<Props> = ({
  currentState,
  onSelectState,
  onOpenHints,
}) => {
  ...
  const currentIndex = G_STATES.findIndex(s => s.id === currentState);

  useEffect(() => {
    let timer: ReturnType<typeof setInterval>;
    if (isPlaying) {
      timer = setInterval(() => {
        const nextIdx = (currentIndex + 1) % G_STATES.length;
        onSelectState(G_STATES[nextIdx].id);
      }, 3500 / speed);
    }
    return () => {
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-5 border border-emerald-100/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 self-end md:self-auto">
        <button
          onClick={() => onSelectState(G_STATES[Math.max(0, currentIndex - 1)].id)}
        ...
        <button
          onClick={() => onSelectState(G_STATES[Math.min(G_STATES.length - 1, currentIndex + 1)].id)}
          disabled={currentIndex === G_STATES.length - 1}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 disabled:opacity-40"
        >
