...
const ODD_STATES: { id: ProofState; label: string; step: number; desc: string }[] = [
  { id: 'inspect', label: 'Inspect', step: 1, desc: 'Identify 1 block as the 1 × 1 core square' },
  { id: 'manipulate', label: 'Manipulate', step: 2, desc: 'Wrap 3 blocks as an L-shaped gnomon to make 2 × 2' },
  { id: 'preserve', label: 'Preserve', step: 3, desc: 'Observe that each step preserves the square geometry' },
  { id: 'connect', label: 'Connect', step: 4, desc: 'Count that layer k always has exactly (2k - 1) blocks' },
  { id: 'conclude', label: 'Conclude', step: 5, desc: 'Conclude 1 + 3 + ... + (2n - 1) = n²' },
  { id: 'transfer', label: 'Transfer', step: 6, desc: 'Solve rapid mental squares by odd summation' },
];

export const OddSumAnimationBar: React.FC<Props> = ({
  currentState,
  onSelectState,
  onOpenHints,
}) => {
  ...
  const currentIndex = ODD_STATES.findIndex(s => s.id === currentState);

  useEffect(() => {
    let timer: ReturnType<typeof setInterval>;
    if (isPlaying) {
      timer = setInterval(() => {
        const nextIdx = (currentIndex + 1) % ODD_STATES.length;
        onSelectState(ODD_STATES[nextIdx].id);
      }, 3500 / speed);
    }
    return () => {
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-5 border border-amber-100/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 self-end md:self-auto">
        <button
          onClick={() => onSelectState(ODD_STATES[Math.max(0, currentIndex - 1)].id)}
        ...
        <button
          ...
          className="p-2.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white shadow-md shadow-amber-500/20"
        >
          {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 fill-white" />}
        </button>

        <button
          onClick={() => onSelectState(ODD_STATES[Math.min(ODD_STATES.length - 1, currentIndex + 1)].id)}
          disabled={currentIndex === ODD_STATES.length - 1}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 disabled:opacity-40"
        >
