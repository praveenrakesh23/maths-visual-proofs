...
const H_STATES: { id: ProofState; label: string; step: number; desc: string }[] = [
  { id: 'inspect', label: 'Inspect', step: 1, desc: 'Identify amplitude A and equilibrium position x=0' },
  { id: 'manipulate', label: 'Manipulate', step: 2, desc: 'Adjust amplitude, frequency, and phase parameters' },
  { id: 'preserve', label: 'Preserve', step: 3, desc: 'Verify Total Energy E = K + U remains constant' },
  { id: 'connect', label: 'Connect', step: 4, desc: 'Relate circular phasor projection to cosine curve' },
  { id: 'conclude', label: 'Conclude', step: 5, desc: 'Derive ẍ + ω²x = 0 and ω = √(k/m)' },
  { id: 'transfer', label: 'Transfer', step: 6, desc: 'Test variable stiffness and mass systems' },
];

export const SHMAnimationBar: React.FC<Props> = ({
  currentState,
  onSelectState,
  onOpenHints,
}) => {
  ...
  const currentIndex = H_STATES.findIndex(s => s.id === currentState);

  useEffect(() => {
    let timer: ReturnType<typeof setInterval>;
    if (isPlaying) {
      timer = setInterval(() => {
        const nextIdx = (currentIndex + 1) % H_STATES.length;
        onSelectState(H_STATES[nextIdx].id);
      }, 3500 / speed);
    }
    return () => {
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-5 border border-amber-100/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 self-end md:self-auto">
        <button
          onClick={() => onSelectState(H_STATES[Math.max(0, currentIndex - 1)].id)}
        ...
        <button
          ...
          className="p-2.5 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white shadow-md shadow-amber-500/20"
        >
          {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4 fill-white" />}
        </button>

        <button
          onClick={() => onSelectState(H_STATES[Math.min(H_STATES.length - 1, currentIndex + 1)].id)}
          disabled={currentIndex === H_STATES.length - 1}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 disabled:opacity-40"
        >
