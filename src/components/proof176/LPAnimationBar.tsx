...
const LP_STATES: { id: ProofState; label: string; step: number; desc: string }[] = [
  { id: 'inspect', label: 'Inspect', step: 1, desc: 'Identify linear objective Z and constraint inequalities' },
  { id: 'manipulate', label: 'Manipulate', step: 2, desc: 'Intersect half-planes to form feasible convex polygon' },
  { id: 'preserve', label: 'Preserve', step: 3, desc: 'Sweep parallel objective level lines through the region' },
  { id: 'connect', label: 'Connect', step: 4, desc: 'Evaluate objective function values at corner vertices' },
  { id: 'conclude', label: 'Conclude', step: 5, desc: 'Synthesize Fundamental Theorem of Linear Programming' },
  { id: 'transfer', label: 'Transfer', step: 6, desc: 'Test cost minimization and multi-resource allocation' },
];

export const LPAnimationBar: React.FC<Props> = ({
  currentState,
  onSelectState,
  onOpenHints,
}) => {
  ...
  const currentIndex = LP_STATES.findIndex(s => s.id === currentState);

  useEffect(() => {
    let timer: ReturnType<typeof setInterval>;
    if (isPlaying) {
      timer = setInterval(() => {
        const nextIdx = (currentIndex + 1) % LP_STATES.length;
        onSelectState(LP_STATES[nextIdx].id);
      }, 3500 / speed);
    }
    return () => {
  ...
  return (
    <div className="bg-white rounded-3xl p-4 md:p-5 border border-violet-100/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2 self-end md:self-auto">
        <button
          onClick={() => onSelectState(LP_STATES[Math.max(0, currentIndex - 1)].id)}
        ...
        <button
          onClick={() => onSelectState(LP_STATES[Math.min(LP_STATES.length - 1, currentIndex + 1)].id)}
          disabled={currentIndex === LP_STATES.length - 1}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 disabled:opacity-40"
        >
